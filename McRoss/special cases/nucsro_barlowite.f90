...
PROGRAM NucSRO  !!!!BARLOWITE version!!!!
...
!  Title
    ...
    CALL nucsro_run_params(conc,aflg,anchor,simflg,n,ii_level,norm)
...
!        END DO
!*******************************BARLOWITE**********************************
    ! choose Cu positions in barlowite - per group
        DO iatom = 1,n,3
            CALL RANDOM_NUMBER(r)
            ihit = iatom + INT(r*3)
            imp(ihit) = 1
...
!*******************************BARLOWITE**********************************
    ...
    ! plot fit
                ...
                ytitle = 'Cross-Section (barns st^{-1} '//TRIM(norm)//'^{-1})'
...
!*******************************BARLOWITE**********************************
    ! choose barlowite triangular group
    ! check that there is one and only one Cu per group
            CALL RANDOM_NUMBER(r)
            iatom = INT(r * DBLE(nimp))*3 + 1
            icount = 0
            DO ihit = 0,2
                IF (imp(iatom + ihit) == 1) THEN
                    new_host = iatom + ihit
                    icount = icount + 1
                    IF (icount > 1) THEN
                        PRINT*, 'NUCSRO: ERROR - more than one impurity found in group'
                        STOP
                    END IF
                END IF
            END DO
            IF (icount /= 1) THEN
                PRINT*, 'NUCSRO: ERROR - no impurities found in group'
                STOP
            END IF
    ! choose new Cu position
30          CALL RANDOM_NUMBER(r)
            ihit = INT(r * 3d0)
            IF (imp(iatom + ihit) == 1) GO TO 30
            new_imp = iatom + ihit
!*******************************BARLOWITE**********************************
...
SUBROUTINE NucSRO_run_params(conc,aflg,anchor,simflg,numtot,ii_level,norm)
...
LOGICAL             :: aflg, simflg
CHARACTER(LEN=255)  :: line, ans, norm
