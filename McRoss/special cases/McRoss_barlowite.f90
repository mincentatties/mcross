...
INTEGER, ALLOCATABLE :: imp(:),shell_from_r(:), shellindex(:)
REAL(8)              :: shellbin = 0.0001, latt(3), weight, rfactor
INTEGER              :: numat, shell_multiplier, maxmoves, numsims
INTEGER              :: nunitc(3)
...
!------------------------------------------------------------------------
    SUBROUTINE read_data(aflg,anchor,numd)
        ...
        DO iimp = 1, ncentres
            ...
            DO j = 1, atoms
                ...
                IF (imp(j) == 1) THEN
                    nb(ish) = nb(ish) + 1
!*******************************BARLOWITE**********************************
! these lines important for Barlowite version - they catch Cu atoms in same group of three
                    IF (ish == 1 .OR. ish == 2) THEN
                        PRINT*, i, j, dist(i,j), imp(i), imp(j) 
                        pause
                    END IF
!*******************************BARLOWITE**********************************
                END IF
...
!----------------------------------------------------------------------
!      
    SUBROUTINE gnuplot_fitmonitor(ffile,xtitle,ytitle,title)

    !  Generic gnuplot batch file operation routine developed for use with
    !  nucsro and magsro

    IMPLICIT NONE
    CHARACTER (LEN=255) :: ffile,xtitle,ytitle,title,input_pfile,ofile

        ofile = TRIM(ffile)//'.pdf'

        input_pfile = 'plot "'//TRIM(dfile)//'" using 1:2:3 ls 2 pt 7 ps '//TRIM(ptsize)//' lc "blue", "' & 
                   //TRIM(ffile)//'" smooth csplines w lines ls 1 lw 2 lc "red"'
        IF(INDEX(ffile,'.co') > 0) input_pfile = 'plot "'//TRIM(ffile)//'" using 1:2:3 pt 7 ps '//TRIM(ptsize)//' lc "blue"'
        ...
        IF(INDEX(ffile,'.co') > 0) WRITE(10,*) 'set xzeroaxis lw 2'
